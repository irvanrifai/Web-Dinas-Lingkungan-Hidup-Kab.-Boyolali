...
function tambah($data){
	...
	$queri="INSERT INTO kinerja_sop_pengaduan VALUES 
			('', '$sop_pengaduan')";
...
function hapus($id){
	global $konek;
	mysqli_query($konek, "DELETE FROM kinerja_sop_pengaduan WHERE id=$id");
	return mysqli_affected_rows($konek);
}
function edit($data){
	global $konek;
	//ambil semua data yang dikirimkan form
	$id=$data["id"];
	$sop_pengaduanLama= htmlspecialchars($data["sop_pengaduan"]);
	
	//jika user mengubah gambar
	if($_FILES['sop_pengaduan']['error'] === 4){
		$sop_pengaduan = $sop_pengaduanLama;
	}
	else{
		$sop_pengaduan = upload();
	}
	...
	$querii="UPDATE kinerja_sop_pengaduan SET sop_pengaduan='$sop_pengaduan' WHERE id=$id";
...
function upload(){
	$namaFile = $_FILES['sop_pengaduan']['name'];
	$ukuranFile = $_FILES['sop_pengaduan']['size'];
	$error = $_FILES['sop_pengaduan']['error'];
	$tmpName = $_FILES['sop_pengaduan']['tmp_name'];
