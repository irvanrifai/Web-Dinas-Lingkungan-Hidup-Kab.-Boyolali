...
function tambah($data){
	...
	$queri="INSERT INTO kinerja_std_pelayanan VALUES 
			('', '$std_pelayanan')";
...
function hapus($id){
	global $konek;
	mysqli_query($konek, "DELETE FROM kinerja_std_pelayanan WHERE id=$id");
	return mysqli_affected_rows($konek);
}
function edit($data){
	global $konek;
	//ambil semua data yang dikirimkan form
	$id=$data["id"];
	$std_pelayananLama= htmlspecialchars($data["std_pelayanan"]);
	
	//jika user mengubah gambar
	if($_FILES['std_pelayanan']['error'] === 4){
		$std_pelayanan = $std_pelayananLama;
	}
	else{
		$std_pelayanan = upload();
	}
	...
	$querii="UPDATE kinerja_std_pelayanan SET std_pelayanan='$std_pelayanan' WHERE id=$id";
...
function upload(){
	$namaFile = $_FILES['std_pelayanan']['name'];
	$ukuranFile = $_FILES['std_pelayanan']['size'];
	$error = $_FILES['std_pelayanan']['error'];
	$tmpName = $_FILES['std_pelayanan']['tmp_name'];
