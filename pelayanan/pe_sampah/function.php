...
function tambah($data){
	...
	$queri="INSERT INTO pelayanan_pe_sampah VALUES 
			('', '$pe_sampah')";
...
function hapus($id){
	global $konek;
	mysqli_query($konek, "DELETE FROM pelayanan_pe_sampah WHERE id=$id");
	return mysqli_affected_rows($konek);
}
function edit($data){
	global $konek;
	//ambil semua data yang dikirimkan form
	$id=$data["id"];
	$pe_sampahLama= htmlspecialchars($data["pe_sampah"]);
	$pe_sampah= htmlspecialchars($data["pe_sampah"]);
	
	//jika user mengubah gambar
	if($_FILES['pe_sampah']['error'] === 4){
		$pe_sampah = $pe_sampahLama;
	}
	else{
		$pe_sampah = upload();
	}
	...
	$querii="UPDATE pelayanan_pe_sampah SET pe_sampah='$pe_sampah' WHERE id=$id";
...
function upload(){
	$namaFile = $_FILES['pe_sampah']['name'];
	$ukuranFile = $_FILES['pe_sampah']['size'];
	$error = $_FILES['pe_sampah']['error'];
	$tmpName = $_FILES['pe_sampah']['tmp_name'];
