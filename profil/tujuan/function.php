...
function tambah($data){
	...
	$queri="INSERT INTO profil_tujuan VALUES 
			('', '$tujuan')";
...
function hapus($id){
	global $konek;
	mysqli_query($konek, "DELETE FROM profil_tujuan WHERE id=$id");
	return mysqli_affected_rows($konek);
}
function edit($data){
	global $konek;
	//ambil semua data yang dikirimkan form
	$id=$data["id"];
	$tujuanLama= htmlspecialchars($data["tujuan"]);
	
	//jika user mengubah gambar
	if($_FILES['tujuan']['error'] === 4){
		$tujuan = $tujuanLama;
	}
	else{
		$tujuan = upload();
	...
	$querii="UPDATE profil_tujuan SET tujuan='$tujuan' WHERE id=$id";
...
function upload(){
	$namaFile = $_FILES['tujuan']['name'];
	$ukuranFile = $_FILES['tujuan']['size'];
	$error = $_FILES['tujuan']['error'];
	$tmpName = $_FILES['tujuan']['tmp_name'];
